...
<div class="px-20 py-10">
    <h1 class="text-3xl md:text-4xl font-serif font-bold mb-6">{{$title}}</h1>

    <form action="{{isset($journals) ? route('modify.journal' , $journals->id) : route('add.journal')}}" method="POST" enctype="multipart/form-data"
        ...
        <div>
            ...
            <input type="text" id="title" name="title" 
                   value="{{ old('title', $journals->title ?? '') }}"
        ...
        <div>
            ...
            <input type="text" id="category" name="category" 
                   value="{{ old('category', $journals->category ?? '') }}"
                   class="w-full border border-graphite/30 rounded-lg px-4 py-2 focus:ring-wine focus:border-wine/50">
        </div>
        ...
        <div>
            ...
            <textarea id="excerpt" name="excerpt" rows="3"
                      ...
                      class="w-full border border-graphite/30 rounded-lg px-4 py-2 focus:ring-wine focus:border-wine/50">{{ old('excerpt', $journals->excerpt ?? '') }}</textarea>
        </div>
        ...
        <div>
            ...
            <textarea id="content" name="content" rows="10"
                      ...
                      class="w-full border border-graphite/30 rounded-lg px-4 py-2 focus:ring-wine focus:border-wine/50">{{ old('content', $journals->content ?? '') }}</textarea>
        </div>
